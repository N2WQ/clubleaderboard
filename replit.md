...
- **Contest Insights**: Homepage displays three insight boxes that adapt to selected leaderboard tab:
  - **Most Recent Logs**: Shows 5 most recent accepted submissions (always year-independent, regardless of tab selection). Displays callsign (muted text) and contest key as blue clickable link to contest detail page. API: `/api/insights/recent-logs` returns latest 5 submissions ordered by submission date.
  - **Most Active Operators**: Top 5 operators by number of submitted logs with tie handling. Adapts to selected tab (current year, all-time, or historical year). Displays entry count without ranking numbers. Links to operator detail pages. API: `/api/insights/active-operators?year={year}` returns top 5+ (including ties) based on entry count, filtered by year if provided.
  - **Most Competitive Contests**: Top 5 contests by submission count with tie handling. Adapts to selected tab (current year, all-time, or historical year). Displays submission count without ranking numbers. Links to contest detail pages. API: `/api/insights/competitive-contests?year={year}` returns top 5+ (including ties) filtered by season year if provided.
  - Both Active Operators and Competitive Contests APIs fetch 100 records initially, then filter to include all records that tie with 5th place, ensuring complete rankings. Counts are cast to integers to ensure numeric values.
