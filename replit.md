...
- **Contest Insights**: Homepage displays three insight boxes that adapt to selected leaderboard tab:
  - **Most Recent Logs**: Shows 5 most recent operator entries from accepted submissions (always year-independent, regardless of tab selection). Displays member operator callsign (muted text) and contest key as blue clickable link to contest detail page. Queries operator_points table to show individual operators, not station callsigns. API: `/api/insights/recent-logs` returns latest 5 operator entries ordered by submission date.
